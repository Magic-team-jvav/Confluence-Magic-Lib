...
public class AimUtils {
        ...
        if (physics) {
            ...
            for (int blockCheckIdx = 0; blockCheckIdx < 3; blockCheckIdx++) {
                ...
                if (loopBeginLoc.distanceToSqr(loopEndLoc) < 1e-5)
                    break;
                BlockHitResult blockCollInfo = level.clip(new ClipContext(loopBeginLoc, loopEndLoc, ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, target));
                // 无方块碰撞，结束循环
    ...
    /// 瞄准实体后返回弹幕射出的速度向量；使用source的eyePosition为起始位置。
    /// 注意，本方法返回的是速度向量。
    ///
