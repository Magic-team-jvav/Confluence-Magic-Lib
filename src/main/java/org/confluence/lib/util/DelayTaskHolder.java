...
public class DelayTaskHolder {
    ...
    private final Map<Identifier, ITask> runList = new LinkedHashMap<>();
    ...
    public Map<Identifier, ITask> getRunList() {
        return runList;
    }
    ...
    public void addTask(Identifier id, ITask task) {
        runList.put(id, task);
    }
    ...
    public void removeTask(EquipmentSlot slot) {
        ...
        for (Identifier key : new HashSet<>(runList.keySet())) {
    ...
    public boolean containsTask(Identifier id) {
        return runList.containsKey(id);
    }

    /// 如果返回的是空集合就表示该槽位没有任务
    public Set<Identifier> containsTask(EquipmentSlot slot) {
    ...
    /// 如果返回的是空集合就表示该槽位没有任务
    public Set<Identifier> containsTask(InteractionHand handUsed) {
