...
public final class LibStreamCodecUtils {
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, List<Optional<Ingredient>>> INGREDIENTS = Ingredient.OPTIONAL_CONTENTS_STREAM_CODEC.apply(ByteBufCodecs.list());
    ...
    public static final StreamCodec<ByteBuf, Vector4fc> VECTOR4F = StreamCodec.composite(
            ByteBufCodecs.FLOAT, Vector4fc::x,
            ByteBufCodecs.FLOAT, Vector4fc::y,
            ByteBufCodecs.FLOAT, Vector4fc::z,
            ByteBufCodecs.FLOAT, Vector4fc::w,
            Vector4f::new
    );
    ...
    public static <B extends ByteBuf, V> StreamCodec<B, TagKey<V>> tagKey(ResourceKey<Registry<V>> resourceKey) {
        return new StreamCodec<>() {
            public TagKey<V> decode(B buffer) {
                return TagKey.create(resourceKey, Identifier.STREAM_CODEC.decode(buffer));
            }

            public void encode(B buffer, TagKey<V> tagKey) {
                Identifier.STREAM_CODEC.encode(buffer, tagKey.location());
            }
        };
    }
