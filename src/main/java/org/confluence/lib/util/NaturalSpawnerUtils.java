...
public final class NaturalSpawnerUtils {
    ...
    public static ChunkSpawnData getChunkSpawnData(ResourceKey<Level> dimension, ChunkPos pos) {
        Long2ObjectMap<ChunkSpawnData> map = CHUNK_DATA.get(dimension);
        return map == null ? ChunkSpawnData.DEFAULT : map.getOrDefault(pos.pack(), ChunkSpawnData.DEFAULT);
    }
    ...
    public static void init(MinecraftServer server) {
        Registry<LevelStem> registry = server.registries().compositeAccess().lookupOrThrow(Registries.LEVEL_STEM);
        for (ResourceKey<LevelStem> key : registry.registryKeySet()) {
            ResourceKey<Level> dimension = ResourceKey.create(Registries.DIMENSION, key.identifier());
            ...
            for (ServerPlayer player : server.getPlayerList().getPlayers()) {
                ...
                if (!CHUNK_DATA.containsKey(dimension)) {
                    ...
                    if (UNKNOWN_DIMENSIONS.add(dimension)) {
                        ConfluenceMagicLib.LOGGER.warn("Why there's a new dimension '{}' here?", dimension.identifier());
                    }
                    continue;
                }
