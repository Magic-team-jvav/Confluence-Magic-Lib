...
public class PlayerAnimationState extends AnimationState<PlayerGeoAnimatable> {
    ...
    public static void allStack(ModelPart part, CoreGeoBone bone) {
    ...
    public static void scaleStack(ModelPart part, CoreGeoBone bone) {
        ModelPropertiesCallback.pos(part, bone, false);
        ModelPropertiesCallback.rot(part, bone, false);
        ModelPropertiesCallback.scale(part, bone, true);
    }

    public void legStack(ModelPart part, CoreGeoBone bone) {
        ModelPropertiesCallback.pos(part, bone, true);
        ModelPropertiesCallback.scale(part, bone, true);
        if (!isMoving()) {
            ModelPropertiesCallback.rot(part, bone, false);
    ...
    public interface ModelPropertiesCallback {
        void update(ModelPart part, CoreGeoBone bone);

        static void pos(ModelPart part, CoreGeoBone bone, boolean stack) {
            if (stack) {
                part.x += bone.getPosX();
                part.y -= bone.getPosY();
                part.z += bone.getPosZ();
        ...
        static void rot(ModelPart part, CoreGeoBone bone, boolean stack) {
            if (stack) {
        ...
        static void scale(ModelPart part, CoreGeoBone bone, boolean stack) {
            if (stack) {
