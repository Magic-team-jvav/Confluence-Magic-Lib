package org.confluence.lib.integration.apothic;
...
public class ApothicHelper {
    ...
    public static void preset(Map<Holder<Attribute>, Holder<Attribute>> map) {
        if (LOADED) {
            map.put(ConfluenceMagicLib.CRITICAL_CHANCE, BuiltInRegistries.ATTRIBUTE.getHolder(ApothicHelper.CRIT_CHANCE).orElse(null));
            map.put(ConfluenceMagicLib.RANGED_VELOCITY, BuiltInRegistries.ATTRIBUTE.getHolder(ApothicHelper.ARROW_VELOCITY).orElse(null));
            map.put(ConfluenceMagicLib.RANGED_DAMAGE, BuiltInRegistries.ATTRIBUTE.getHolder(ApothicHelper.ARROW_DAMAGE).orElse(null));
            map.put(ConfluenceMagicLib.DODGE_CHANCE, BuiltInRegistries.ATTRIBUTE.getHolder(ApothicHelper.DODGE_CHANCE).orElse(null));
            map.put(ConfluenceMagicLib.ARMOR_PENETRATION, BuiltInRegistries.ATTRIBUTE.getHolder(ApothicHelper.ARMOR_PIERCE).orElse(null));
        }
    }
}
