...
public class SimpleTemplatePiece extends TemplateStructurePiece {
    ...
    public SimpleTemplatePiece(StructureTemplateManager structureTemplateManager, CompoundTag tag) {
        super(ConfluenceMagicLib.SIMPLE_TEMPLATE_PIECE.get(), tag, structureTemplateManager, _ -> makeSettings(
                tag.getBooleanOr("OW", false),
                tag.getBooleanOr("IE", false),
                tag.getBooleanOr("AW", false),
                Rotation.valueOf(tag.getString("Rot").orElseThrow())
        ));
    }
