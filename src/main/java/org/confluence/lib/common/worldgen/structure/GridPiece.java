...
public class GridPiece extends StructurePiece {
    ...
    public static final Codec<List<Tuple<BlockPos, Identifier>>> FEATURES_CODEC = LibCodecUtils.tuple(BlockPos.CODEC, Identifier.CODEC).listOf();

    private final ChunkPos startPos;
    private final List<Tuple<Integer, IntArrayList>> blockMap;
    public List<BlockState> blockList;
    private final List<Tuple<BlockPos, Identifier>> features;

    public GridPiece(ChunkPos startPos, int minY, int maxY, Object2IntMap<BlockPos> blockMap, Map<BlockPos, Identifier> features) {
    ...
    private void convertToList(Object2IntMap<BlockPos> blockMap, Map<BlockPos, Identifier> features) {
    ...
    public GridPiece(CompoundTag tag) {
        ...
        this.startPos = ChunkPos.unpack(tag.getLong("StartPos").orElse(0L));
    ...
    protected void addAdditionalSaveData(StructurePieceSerializationContext context, CompoundTag tag) {
        tag.putLong("StartPos", startPos.pack());
    ...
    public void postProcess(WorldGenLevel level, StructureManager structureManager, ChunkGenerator generator, RandomSource random, BoundingBox box, ChunkPos chunkPos, BlockPos pos) {
        ...
        for (Tuple<BlockPos, Identifier> pair : features) {
            configuredFeatures.get(pair.getB())
    ...
    public static void addPieces(Object2IntMap<BlockPos> blockMap, List<BlockState> blockList, Map<BlockPos, Identifier> featureMap, StructurePiecesBuilder builder) {
        Map<ChunkPos, Map<BlockPos, Identifier>> sliceMap = new HashMap<>();
        for (Map.Entry<BlockPos, Identifier> posEntry : featureMap.entrySet()) {
            BlockPos blockPos = posEntry.getKey();
            sliceMap.computeIfAbsent(ChunkPos.containing(blockPos), map -> new HashMap<>()).put(blockPos, posEntry.getValue());
        }
        ...
        for (Object2IntMap.Entry<BlockPos> posEntry : blockMap.object2IntEntrySet()) {
            ...
            if (y > maxY) maxY = y;
            if (y < minY) minY = y;
            gridMap.computeIfAbsent(ChunkPos.containing(blockPos), map -> new Object2IntOpenHashMap<>()).put(blockPos, posEntry.getIntValue());
        }
