...
public abstract class EitherAmountContainerMenu4x<I extends MenuRecipeInput, R extends EitherAmountRecipe4x<I>, S extends ToggleAmountResultSlot<R>, A extends ContainerLevelAccess> extends AbstractContainerMenu implements IMultiRecipeMenu<R> {
    ...
    public void removed(Player player) {
        ...
        access.execute((_, _) -> clearContainer(player, input));
    }
    ...
    public void slotsChanged(Container container) {
        ...
        access.execute((_, _) -> {
            if (player instanceof ServerPlayer serverPlayer) {
                setRecipes(serverPlayer.level().getServer().getRecipeManager().recipeMap().getRecipesFor(recipeType, input, player.level()).toList());
    ...
    public ItemStack quickMoveStack(Player player, int pIndex) {
        ...
        if (slot.hasItem()) {
            ...
            if (pIndex == 0) { // resultSlot
                access.execute((_, _) -> slotItem.getItem().onCraftedBy(slotItem, player));
    ...
    @Override
    public void setRecipes(List<RecipeHolder<R>> recipes) {
