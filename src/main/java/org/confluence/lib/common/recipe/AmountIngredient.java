...
public record AmountIngredient(Ingredient ingredient, int amount) implements ICustomIngredient {
    public static final Ingredient EMPTY = new AmountIngredient(new Ingredient(null), 0).toVanilla();
    public static final MapCodec<AmountIngredient> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            Ingredient.CODEC.fieldOf("ingredient").forGetter(AmountIngredient::ingredient),
            ExtraCodecs.POSITIVE_INT.lenientOptionalFieldOf("count", 1).forGetter(AmountIngredient::amount)
    ...
    public static Ingredient of(int amount, TagKey<Item> tag) {
        return new AmountIngredient(Ingredient.of(BuiltInRegistries.ITEM.getOrThrow(tag)), amount).toVanilla();
    }

    public static Ingredient of(ItemStack stack) {
        return new AmountIngredient(Ingredient.of(stack.getItem()), stack.getCount()).toVanilla();
    }
}
